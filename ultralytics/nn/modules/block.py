...
class _DirectionalPConvBranch(nn.Module):
    """Single pinwheel branch that processes one kernel size and fuses directional responses."""

    def __init__(self, c1: int, c2: int, k: int, s: int) -> None:
        super().__init__()
        padding_cfg = [(k, 0, 1, 0), (0, k, 0, 1), (0, 1, k, 0), (1, 0, 0, k)]
        self.pad = nn.ModuleList(nn.ZeroPad2d(cfg) for cfg in padding_cfg)
        self.cw = Conv(c1, c2 // 4, (1, k), s=s, p=0)
        self.ch = Conv(c1, c2 // 4, (k, 1), s=s, p=0)
        self.fuse = Conv(c2, c2, 2, s=1, p=0)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        planar = [self.cw(self.pad[0](x)), self.cw(self.pad[1](x)), self.ch(self.pad[2](x)), self.ch(self.pad[3](x))]
        return self.fuse(torch.cat(planar, dim=1))
...
class PConv(nn.Module):  
    ...
    def __init__(self, c1, c2, k, s):
        ...
        if isinstance(k, (list, tuple)):
            kernel_sizes = [int(ks) for ks in k]
        else:
            base = int(k)
            extra = base + 2 if base % 2 == 1 else base + 1
            kernel_sizes = [base, extra] if base <= 5 else [base]

        self.kernel_sizes = sorted({ks for ks in kernel_sizes if ks > 0 and ks % 2 == 1})
        self.branches = nn.ModuleList([_DirectionalPConvBranch(c1, c2, ks, s) for ks in self.kernel_sizes])

        self.num_scales = len(self.branches)
        if self.num_scales > 1:
            reduction = max(c1 // 4, 1)
            self.attn_reduce = nn.Conv2d(c1, reduction, kernel_size=1, bias=True)
            self.attn_expand = nn.Conv2d(reduction, self.num_scales, kernel_size=1, bias=True)
            self.act = nn.SiLU(inplace=True)
            self.pool = nn.AdaptiveAvgPool2d(1)
            self.softmax = nn.Softmax(dim=1)
        else:
            self.attn_reduce = None
    ...
    def forward(self, x):
        ...
        pooled = self.pool(x)
        weights = self.attn_expand(self.act(self.attn_reduce(pooled)))
        weights = self.softmax(weights.view(x.size(0), self.num_scales, -1)).view(x.size(0), self.num_scales, 1, 1, 1)
...
class _RGBContextUnit(nn.Module):
    """Bi-branch RGB refinement unit with background context and detail enhancement."""

    def __init__(self, c_in, cm, act=nn.ReLU(), use_lightconv=False):
        super().__init__()
        base_block = LightConv if use_lightconv else Conv
        mid_gating = max(cm // 2, 1)

        self.pre = base_block(c_in, cm, k=3, act=act)
        self.background = nn.Sequential(
            Conv(cm, cm, k=1, act=act),
            Conv(cm, cm, k=3, d=2, act=act)
        )

        self.detail = nn.Sequential(
            Conv(cm, cm, k=3, act=act),
            Conv(cm, cm, k=5, g=cm, act=act),
            Conv(cm, cm, k=1, act=False)
        )

        self.gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(cm, mid_gating, 1, bias=True),
            nn.SiLU(inplace=True),
            nn.Conv2d(mid_gating, cm, 1, bias=True),
            nn.Sigmoid()
        )

        self.fuse = Conv(2 * cm, cm, k=1, act=act)

    def forward(self, x):
        base = self.pre(x)
        bg = self.background(base)
        detail = self.detail(base) * self.gate(bg)
        return self.fuse(torch.cat([bg, detail], 1))    
    
class CGDEM(nn.Module):
    def __init__(self, c1, cm, c2, k=3, n=6, lightconv=False, shortcut=False, act=nn.ReLU()):
        super().__init__()
        self.m = nn.ModuleList(
            _RGBContextUnit(c1 if i == 0 else cm, cm, act=act, use_lightconv=lightconv) for i in range(n)
        )
